...
function renderApp(data) {
  ...
  data.skills.forEach((skill) => {
    ...
    // Skill body — collapsed by default
    ...
    skill.levels.forEach((level) => {
      ...
      // Stage header — collapsed by default
      ...
      level.requirements.forEach((req, i) => {
        ...
        // Support both string format and {competency, requirement} object format
        const competencyText = typeof req === "string" ? req : req.competency;
        const requirementText = typeof req === "string" ? null : (req.requirement || null);
        ...
        // Text block holds competency + italicised requirement below
        const textBlock = document.createElement("div");
        textBlock.className = "req-text-block";

        const compSpan = document.createElement("span");
        compSpan.className = `req-text${isChecked ? " done" : ""}`;
        compSpan.id = `rt-${key}`;
        compSpan.textContent = competencyText;
        textBlock.appendChild(compSpan);

        if (requirementText) {
          const reqSpan = document.createElement("span");
          reqSpan.className = "req-detail";
          reqSpan.textContent = requirementText;
          textBlock.appendChild(reqSpan);
        }
        ...
        row.addEventListener("click", () => {
          ...
          compSpan.classList.toggle("done", progress[key]);
